...
class TableBody extends Component {
  ...
  render() {
    ...
    const tableRows = this.props.data.map((data, r) => {
      const tableColumns = this.props.columns.map((column, i) => {
  ...
  handleRowMouseOut = (rowIndex, event) => {
    const { onRowMouseOut } = this.props;

    const targetRow = this.props.data[rowIndex];
    onRowMouseOut && onRowMouseOut(targetRow, event);
  }

  handleRowMouseOver = (rowIndex, event) => {
    const { onRowMouseOver } = this.props;
    const targetRow = this.props.data[rowIndex];
    onRowMouseOver && onRowMouseOver(targetRow, event);
  }
  ...
  handleDblRowClick = row => {
    const { data, onDblRowClick } = this.props;
    onDblRowClick(row);
  }
